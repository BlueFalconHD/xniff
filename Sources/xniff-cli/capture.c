...
int xniff_capture_ring(pid_t pid,
    ...
    const bool bin_only = options->out_bin;
    if (bin_only) {
        if (strcmp(options->out_bin_path, "-") == 0) {
            out_bin_fp = stdout;
        } else {
            out_bin_fp = fopen(options->out_bin_path, "wb");
            if (!out_bin_fp) {
    ...
    for (;;) {
        ...
        size_t consumed = 0;
        while (stream_len - consumed >= sizeof(xniff_ipc_v2_entry_hdr_t)) {
            const uint8_t *frame = stream + consumed;
            ...
            if (stream_len - consumed < frame_len) break;

            if (bin_only) {
                if (write_bin_record(out_bin_fp, frame, frame_len) != 0) {
            ...
            const uint8_t *body = frame + sizeof(eh);
