...
layers are **Hex**, **Raw XPC**, **Foundation NSXPC**, **Swift XPC Codable**, and
**Core Data**. Hex is the root inspector, so every semantic layer can navigate
back to the bytes that produced it. Foundation parses the `f`, `root`, `ool`,
`proxynum`, `replysig`, and `sequence` transport envelope; reconstructs request
and reply invocations; labels arguments from their Objective-C type encodings;
and recognizes proxy release and progress control frames. The Swift XPC layer
decodes the `_CodableBody` encoding graph, including keyed, unkeyed, and
single-value containers, scalar values, out-of-line data, and unambiguous
`XPCCodableObject` references. Core Data then hides `NSCoreDataXPCMessage` and
its nested archive framing. You can select a lower layer at any time to audit
that transformation. Private Core Data result buffers whose format is not yet
understood are labeled as opaque rather than presented as a successful decode.
